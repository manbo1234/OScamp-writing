...
![`println!` 背后的完整调用链](images/no_std_article/01-println-call-chain.png)

如果按课件里的实验先跑一遍 `strace ./hello 2>&1 | wc -l`，你会更直观看到：普通 `println!` 版本会带出一串初始化和收尾 syscall，而 `no_std` 版本把路径压缩到极少数必要调用时，程序边界一下子就清楚了。

![`strace` 对比：47 次 syscall vs 2 次 syscall](images/no_std_article/05-strace-compare.png)
...
![Rust 三层库关系与 `#![no_std]` 的影响](images/no_std_article/02-rust-lib-stack.png)
...
![系统调用作为用户态进入内核态的合法入口](images/no_std_article/03-syscall-gate.png)
...
![从 `asm!` 到 `syscall3` 的抽象分层](images/no_std_article/04-syscall-abstraction.png)
...
![Bump Allocator 的内存切片示意](images/no_std_article/06-bump-allocator.png)
...
![内存对齐前后的地址变化](images/no_std_article/07-alignment.png)
...
![Free List 复用空洞示意](images/no_std_article/08-free-list.png)
...
![Handle Table：数组索引即凭证](images/no_std_article/09-handle-table.png)
